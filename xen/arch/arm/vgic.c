...
int domain_vgic_init(struct domain *d, unsigned int nr_spis)
{
    ...
    if ( nr_spis > (1020 - NR_LOCAL_IRQS) ) {
        if ( nr_spis > ESPI_MAX_INTID )
            ...
            nr_spis = 1020 - NR_LOCAL_IRQS;
    }

    d->arch.vgic.nr_espis = NR_IRQS;
    ...
    d->arch.vgic.allocated_irqs =
        xzalloc_array(unsigned long, BITS_TO_LONGS(vgic_num_irqs(d) + d->arch.vgic.nr_espis));
...
bool vgic_reserve_virq(struct domain *d, unsigned int virq)
{
    ...
    if (is_espi(virq))
        return !test_and_set_bit(virq - ESPI_BASE_INTID + vgic_num_irqs(d), d->arch.vgic.allocated_irqs);
...
int vgic_allocate_virq(struct domain *d, bool spi)
{
    ...
    {
        first = 32;
        end = vgic_num_irqs(d) + d->arch.vgic.nr_espis;
    }
